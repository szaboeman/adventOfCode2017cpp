...
public:
    ...
    void backTrack() {
        ...
        int i=0, act=0, maxSum=0, maxLen=0;
        while (i<edgeList.size() && edgeList[i].first!=0) i++;
            ...
            if (j<edgeList.size() && isGood) {
                ...
                if (result[0].first==0 || result[0].second==0) {
                    ...
                    for (int k=0; k<result.size(); k++) {
                        ...
                        sum=sum+result[k].first+result[k].second;
                    }
                ...
                if (result.size()!=0) {
                    ...
                    i++;
                    while (i<edgeList.size() && edgeList[i].first!=0) i++;
                    //while (i<edgeList.size() && edgeList[i].first!=0 && edgeList[i].second!=0) i++;
                    act=i;
